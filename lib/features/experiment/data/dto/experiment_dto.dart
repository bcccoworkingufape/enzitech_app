...
extension ExperimentDto on ExperimentEntity {
  static ExperimentEntity fromJson(Map json) {
    ...
    return ExperimentEntity(
      ...
      // snapshot da enzima, a configuração (duração, peso, volume, variáveis, fórmulas
      // customizadas) já salva para este experimento — necessário para pré-preencher a
      // tela de edição.
      enzymes: json['experimentEnzymes'] != null
          ? List<EnzymeEntity>.from(
              (json['experimentEnzymes'] as List).map((x) => EnzymeDto.fromExperimentEnzymeConfigJson(x)),
            )
          : null,
    );
  }
