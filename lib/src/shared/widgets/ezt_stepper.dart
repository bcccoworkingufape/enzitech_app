// STEPPER CLASS REWRITING TO SOLVE A BUG.
// ARMSTRONG LOHÃNS - 2022
...
/// The state of a [EZTStep] which is used to control the style of the circle and
/// text.
///
/// See also:
///
...
/// Defines the [EZTStepper]'s main axis.
enum EZTStepperType {
...
class ControlsDetails {
  ...
  /// Index that is active for the surrounding [EZTStepper] widget. This may be
  ...
  /// just activated the transition to a new step, some [EZTStepper.type] values will
...
/// A material step used in [EZTStepper]. The step can have a title and subtitle,
...
@immutable
class EZTStep {
  /// Creates a step for a [EZTStepper].
  ///
  ...
  /// Only [EZTStepperType.horizontal], Optional widget that appears under the [title].
...
///
/// {@tool dartpad}
/// An example the shows how to use the [EZTStepper], and the [EZTStepper] UI
/// appearance.
///
...
///  * [EZTStep]
///  * <https://material.io/archive/guidelines/components/steppers.html>
class EZTStepper extends StatefulWidget {
  ...
  const EZTStepper({
    ...
    required this.steps,
    this.physics,
    this.type = EZTStepperType.vertical,
    this.currentStep = 0,
    this.onStepTapped,
    this.onStepContinue,
    this.onStepCancel,
  ...
  /// [EZTStepperType.horizontal], the content of the current step is displayed
  /// underneath as opposed to the [EZTStepperType.vertical] case where it is
  /// displayed in-between.
  final EZTStepperType type;
  ...
  /// The elevation of this stepper's [Material] when [type] is [EZTStepperType.horizontal].
  ...
  @override
  State<EZTStepper> createState() => _EZTStepperState();
}
...
class _EZTStepperState extends State<EZTStepper> with TickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(EZTStepper oldWidget) {
  ...
  bool _isLabel() {
    for (final EZTStep step in widget.steps) {
      if (step.label != null) {
        return true;
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    assert(() {
      if (context.findAncestorWidgetOfExactType<EZTStepper>() != null) {
        throw FlutterError(
    ...
    switch (widget.type) {
      case EZTStepperType.vertical:
        return _buildVertical();
      case EZTStepperType.horizontal:
